use crate::types::{Keyword, Token, TokenType};
    ...
    while let Some((_, ch)) = chars.next() {
        let token = match ch {
            ...
            '0'..='9' => {
                ...
                let token_type = if string_num.contains('.') {
                    ...
                    match string_num.parse::<i32>() {
                        ...
                        Err(_err) => {
                            return Err(format!("Unable to parse integer [{}]", string_num))
            ...
            'a'..='z' => {
                ...
                for (_, identifier_chr) in
                    chars.take_while_ref(|(_, str_ch)| str_ch.is_alphanumeric())
                {
